...
export function make_planet_at_time(planet, gstates, galaxy_time, static_values) {
    /**
     * Create the state of a planet at the given galaxy_time.
     *
     * @param {number} planet - Planet Index for a planet in question
     * @param {GalaxyStates} gstates - The global states containing planet data.
     * @param {number} galaxy_time - The specific galaxy time for which to obtain the planet state.
     * @param {Object} static_values - The initial static values for the planet.
     * @returns {Object} - The state of the planet with updated information at galaxy_time.
     */
    let planetstate = {};
    // Load Static Values
    ...
    let desc_str = "";
    for (const val of planetstate['desc']) {
        ...
        for (const each of entry.log) {
            if (each.type === "Day Start") {
                ...
                if (/<br\/>/.test(each.text)) {
                    ...
                    listItems.forEach((itemString) => {
                        const textElement = document.createElement("span");
                        textElement.innerHTML = make_mo_link("+ " + itemString);
                        parentCard.appendChild(textElement);
                        parentCard.appendChild(document.createElement("br"));
                    });
                } else {
                    const textElement = document.createElement("span");
                    textElement.innerHTML = make_mo_link(each.text);
                    parentCard.appendChild(textElement);
                    parentCard.appendChild(document.createElement("br"));
                }
